...
export async function ValidateUserToken(req, res, next) {
    ...
    try {
        ...
        if (ValidateToken.rowCount === 0) return res.status(401).send('invalid token');

        // se a hora atual for maior que a hora de expiração do token, então o token é inválido
        if (Date.now() > ValidateToken.rows[0].expiration) return res.status(401).send('token expired');
        next();

    } catch (err) { return res.status(500).send(`Error accessing database during Middleware ValidateUserToken.\n${err}`); }
}
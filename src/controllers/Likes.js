...
export async function GetLikesController(req, res) {
  ...
  try {
    ...
    if (likesCount.rowCount === 0)
      return res
        .status(500)
        .send(`Error during getting likes count from postId: ${postId}`);

    const UsersAsLikedThisPost =
      await LikesRepository.GetUserNameAsLikedThisPost(postId);
    if (UsersAsLikedThisPost.rowCount === 0)
      return res
        .status(500)
        .send(
          `Error during getting users as liked this post from postId: ${postId}`
        );
